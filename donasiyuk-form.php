<?php 
	...
	global $wpdb;
	global $wp;
    // Defensive: ensure popup button label is always defined before any template branch.
    $popup_info_button = 'Next';
    $table_name = $wpdb->prefix . "dyk_campaign";
    $table_name2 = $wpdb->prefix . "dyk_settings";
    $table_name3 = $wpdb->prefix . "dyk_payment_list";
    $table_name4 = $wpdb->prefix . "dyk_users";
    $table_name5 = $wpdb->prefix . "users";
    $table_name6 = $wpdb->prefix . "dyk_donate";
    $table_name7 = $wpdb->prefix . "options";
    $table_name8 = $wpdb->prefix . "dyk_aff_code";
    $table_name9 = $wpdb->prefix . "dyk_blocked_whatsapp";
    ...
    if($row->form_status=='1'){
        $form_text   = (!empty($row->form_text) ? json_decode($row->form_text, true) : []);
        $text1 = $form_text['text'][0];
        $text2 = $form_text['text'][1];
        $text3 = $form_text['text'][2];
        $text4 = $form_text['text'][3];
    ...
    if($row->pixel_status=='1'){
        $fb_event  = (!empty($row->fb_event) ? json_decode($row->fb_event, true) : []);
        $event_1   = $fb_event['event'][0];
        $event_2   = $fb_event['event'][1];
        $event_3   = $fb_event['event'][2];
        if(isset($fb_event['event'][3])){
    ...
    if($row->payment_status=='1'){
    	...
    	$unique_number_value    = (!empty($row->unique_number_value) ? json_decode($row->unique_number_value, true) : []);
    	$method_status 	 = (!empty($row->method_status) ? json_decode($row->method_status, true) : []);
    	$bank_account 	 = (!empty($row->bank_account) ? json_decode($row->bank_account, true) : []);
    ...
    // DRM neutralized
    $popup_info_button = (!empty($row->popup_info_button)) ? $row->popup_info_button : 'Next';
...
<?php } ?>
	...
	<style type="text/css">
		/* Typography matching Campaign & Invoice Pages */
		*, *::before, *::after,
		html, body, input, button, select, textarea,
		h1, h2, h3, h4, h5, h6, p, span, a, div, label,
		.section-box, .main-box, .dyk-program-info-box, .donasiyuk-input, .card-body, .dyk-payment-field,
		.dyk-form-header-card, .card-group, .card-radio-btn, .content_head, .content_sub {
		...
		/* --- Unified Campaign Header Card (Gambar & Nama Donasi Menyatu) --- */
		.dyk-form-header-card {
			background: #ffffff !important;
			border-radius: 12px !important;
		...
		.dyk-form-header-card .section-image {
		...
		.dyk-form-header-card .section-image img {
			width: 100% !important;
		...
		.dyk-hero-back-btn svg {
			stroke: #ffffff !important;
		}

		.dyk-form-header-card .dyk-program-info-box {
			background: #ffffff !important;
			border-radius: 0 !important;
		...
		/* --- Modern Preset Nominal Grid ("Donasi Terbaik Anda") --- */
		.card-group {
		...
		.card-radio-btn .card-input-element {
		...
		.card-radio-btn .content_head.no_desc {
		...
		.card-radio-btn .content_sub.no_desc {
			display: none !important;
		}

		/* Checked / Active Preset */
		.card-input-element:checked + .card,
		.card-style .card-input-element:checked + .card,
		.card-style .card-input-element:checked + .card.card-body {
			border: 2px solid <?php echo $button_color;?> !important;
		...
		.card-input-element + .card .box-checklist {
			display: none !important;
		}

		.card-input-element:checked + .card .box-checklist {
			display: flex !important;
			position: absolute !important;
			top: 3px !important;
			right: 3px !important;
		...
		.card-input-element:checked + .card .box-checklist .checklist svg,
		.card-style .card-input-element:checked + .card .box-checklist .checklist .card-check svg {
		...
		.other_nominal_value input:focus {
			border-color: <?php echo $button_color;?> !important;
			box-shadow: 0 0 0 3px <?php echo $button_color;?>20 !important;
			outline: none !important;
		}

		.other_nominal_value .currency {
			position: absolute !important;
			left: 16px !important;
			top: 50% !important;
		...
		/* Payment dropdown trigger styling */
        .dyk-payment-field{clear:both!important;display:block!important;width:100%!important;margin:20px 0 16px 0!important;position:relative!important;float:none!important}
        .dyk-field-label{display:block!important;width:100%!important;clear:both!important;font-size:14px!important;font-weight:500!important;color:#333!important;margin-bottom:8px!important;text-align:left!important}
        .dyk-select-container{position:relative!important;width:100%!important;display:block!important;clear:both!important}
        .dyk-select-trigger{width:100%!important;max-width:100%!important;min-height:50px!important;height:auto!important;float:none!important;background:#fff!important;border:1px solid #e5e8ec!important;border-radius:6px!important;padding:8px 14px!important;display:flex!important;align-items:center!important;justify-content:space-between!important;cursor:pointer!important;box-sizing:border-box!important;margin:0!important;transition:all .2s ease!important;position:relative!important}
        .dyk-select-trigger:hover,.dyk-select-container.open .dyk-select-trigger{border-color:<?php echo $button_color;?>!important;box-shadow:0 0 0 2px <?php echo $button_color;?>20!important}
        .dyk-select-trigger.set_red{border-color:#f15d5e!important;box-shadow:0 0 0 2px #f15d5e25!important}
        .dyk-trigger-left{display:flex!important;align-items:center!important;gap:12px!important;flex:1!important;overflow:hidden!important;text-align:left!important;margin:0!important;padding:0!important}
        .dyk-trigger-left .box_img_payment{display:flex!important;align-items:center!important;justify-content:center!important;width:36px!important;height:24px!important;flex-shrink:0!important;margin:0!important;padding:0!important;background:transparent!important;border:none!important}
        .dyk-trigger-left .img_payment_selected{position:static!important;max-width:36px!important;max-height:24px!important;width:auto!important;height:auto!important;margin:0!important;padding:0!important;border:none!important;border-radius:0!important;object-fit:contain!important;display:block!important;background:transparent!important}
        .dyk-trigger-left .title_payment{font-size:14px!important;font-weight:500!important;color:#1e293b!important;margin:0!important;padding:0!important;text-transform:capitalize!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important;display:inline-block!important;position:static!important;width:auto!important}
        .dyk-trigger-arrow{display:flex!important;align-items:center!important;justify-content:center!important;color:#64748b!important;margin-left:8px!important;flex-shrink:0!important;transition:transform .2s ease!important}
        .dyk-select-container.open .dyk-trigger-arrow{transform:rotate(180deg)!important}
        .dyk-dropdown-menu{display:none;position:absolute!important;top:calc(100% + 4px)!important;left:0!important;right:0!important;width:100%!important;background:#fff!important;border:1px solid #e2e8f0!important;border-radius:8px!important;box-shadow:0 10px 25px -4px rgba(0,0,0,0.12),0 4px 8px -2px rgba(0,0,0,0.05)!important;max-height:280px!important;overflow-y:auto!important;z-index:10050!important;box-sizing:border-box!important}
        .dyk-select-container.open .dyk-dropdown-menu{display:block!important;animation:dykFadeDown .15s ease-out forwards!important}
        @keyframes dykFadeDown{from{opacity:0;transform:translateY(-4px)}to{opacity:1;transform:translateY(0)}}
        .dyk-dropdown-menu .card_payment{padding:0!important;background:#fff!important;max-height:none!important;overflow:visible!important}
        .dyk-dropdown-menu .title-list{background:#f8fafc!important;border-top:1px solid #f1f5f9!important;border-bottom:1px solid #f1f5f9!important;padding:7px 14px!important;height:auto!important;min-height:auto!important;margin:0!important;cursor:default!important;display:flex!important;align-items:center!important}
        .dyk-dropdown-menu .card-title2{font-size:11px!important;font-weight:700!important;color:#64748b!important;text-transform:none!important;letter-spacing:normal!important;margin:0!important}
        .dyk-dropdown-menu .card-label{display:flex!important;align-items:center!important;height:48px!important;padding:0 14px!important;border-bottom:1px solid #f8fafc!important;border-top:none!important;cursor:pointer!important;transition:background .15s ease!important;margin:0!important;background:#fff!important;width:100%!important;box-sizing:border-box!important}
        .dyk-dropdown-menu .card-label:hover{background:#f1f5f9!important}
        .dyk-dropdown-menu .card-label:last-child{border-bottom:none!important}
        .dyk-dropdown-menu .card-icon{width:45px!important;margin-right:12px!important;display:flex!important;align-items:center!important;flex-shrink:0!important}
        .dyk-dropdown-menu .card-icon img{max-width:45px!important;max-height:24px!important;object-fit:contain!important;display:block!important}
        .dyk-dropdown-menu .card-text{font-size:13px!important;font-weight:500!important;color:#1e293b!important;flex:1!important;text-align:left!important;margin:0!important}
        .dyk-dropdown-menu .card-radio{display:none!important}
        .dyk-dropdown-menu .card-radio:checked~.card-text,.dyk-dropdown-menu .card-label.active .card-text{color:<?php echo $button_color;?>!important;font-weight:600!important}
        .dyk-dropdown-menu .card-radio:checked~.card-check,.dyk-dropdown-menu .card-label.active .card-check{display:inline-flex!important;align-items:center!important}
        .dyk-dropdown-menu .card-check{display:none;margin-left:auto!important}
        .dyk-dropdown-menu .card-check svg{fill:<?php echo $button_color;?>!important;width:18px!important;height:18px!important}
        #simple-popup{position:fixed;top:0;bottom:0;left:0;right:0;z-index:100001;background:rgba(15,23,42,0.45);backdrop-filter:blur(2px);-webkit-backdrop-filter:blur(2px);display:flex;align-items:center;justify-content:center}
        #simple-popup-backdrop,.simple-popup-backdrop-content{position:fixed;top:0;bottom:0;left:0;right:0;z-index:100000}
        #simple-popup,#simple-popup-backdrop,#simple-popup-backdrop.hide-it,#simple-popup.hide-it{-webkit-transition-property:opacity;-moz-transition-property:opacity;-ms-transition-property:opacity;-o-transition-property:opacity;transition-property:opacity}
        #simple-popup-backdrop.hide-it,#simple-popup.hide-it{opacity:0}
        #simple-popup,#simple-popup-backdrop{opacity:1}
        .simple-popup-content{border-radius:14px;position:relative;max-height:85vh;width:440px !important;max-width:92% !important;z-index:100002;padding:0 !important;background:#ffffff;overflow:hidden;box-shadow:0 20px 25px -5px rgba(0,0,0,0.15),0 8px 10px -6px rgba(0,0,0,0.1);border:1px solid #e2e8f0;display:flex;flex-direction:column}
        .simple-popup-content .close{position:absolute;right:14px;top:12px;width:32px;height:32px;display:flex;align-items:center;justify-content:center;cursor:pointer;z-index:10;border-radius:6px;transition:all .15s ease}
        .simple-popup-content .close:hover{background:#f1f5f9}
        .simple-popup-content .close::before{display:inline-block;text-align:center;content:"\00d7";font-size:22px;font-weight:400;color:#94a3b8;line-height:1;transition:color .15s ease}
        .simple-popup-content .close:hover::before{color:#1e293b}
        .dyk-modal-header{padding:16px 20px;border-bottom:1px solid #f1f5f9;background:#ffffff;display:flex;align-items:center;justify-content:space-between;flex-shrink:0}
        .dyk-modal-title{margin:0;font-size:16px;font-weight:700;color:#0f172a;letter-spacing:-0.2px;text-align:left}
        .card_payment{max-width:100%;background-color:#ffffff;padding-top:0 !important;overflow-y:auto;max-height:calc(85vh - 58px)}
        .title-list{background:#f8fafc !important;border-top:1px solid #f1f5f9 !important;border-bottom:1px solid #f1f5f9 !important;padding:8px 20px !important;height:auto !important;min-height:auto !important;margin-top:0 !important;cursor:default !important;display:flex !important;align-items:center !important}
        .card-title2{width:100%;margin:0;text-align:left;font-size:13px;color:#475569;font-weight:600;text-transform:none;letter-spacing:normal}
        .card-label{display:flex;align-items:center;height:54px;border-top:none;border-bottom:1px solid #f8fafc;padding:0 20px;cursor:pointer;transition:background .15s ease}
        .card-label:hover{background:#f8fafc}
        .card-label:last-child{border-bottom:none}
        .card-icon{width:55px;margin-right:16px;text-align:left;display:flex;align-items:center;flex-shrink:0}
        .card-icon img{max-width:55px;max-height:28px;object-fit:contain;display:block}
        .card-text{color:#1e293b;font-size:14px;font-weight:500;flex:1;text-align:left}
        .card-radio{display:none;margin-left:auto}
        .card-radio:checked~.card-text{color:<?php echo $button_color;?>;font-weight:600}
        .card-radio:checked~.card-check{display:inline-flex;align-items:center}
        .card-check{display:none;margin-left:auto}
        .card-check svg{fill:<?php echo $button_color;?> !important;width:20px;height:20px}
        a:active,a:focus,a:visited{box-shadow:none!important;outline:0;box-shadow:0 4px 15px 0 rgba(0,0,0,.1)}.form-group label{font-size:14px}.donasiyuk-input{margin:0 0 16px 0}.donasiyuk-input input,.donasiyuk-input textarea{font-family:Roboto,sans-serif;outline:0;background:#fff;width:100%;padding:15px;box-sizing:border-box;font-size:16px;font-weight:bold;border:1px solid #e5e8ec!important;border-radius:4px;transition:all .2s ease}.donasiyuk-input input[type=email],.donasiyuk-input input[type=number],.donasiyuk-input input[type=tel],.donasiyuk-input input[type=text]{height:50px}.donasiyuk-input input:focus,.donasiyuk-input input:visited,.donasiyuk-input textarea:focus,.donasiyuk-input textarea:visited{border:1px solid #719eca!important}.donasiyuk-input.anonim{padding-top:5px;padding-bottom:10px}.donasiyuk-input.comment{padding-top:0;margin-top:-10px}.donasiyuk-input .donation_button_now{margin-top:5px;margin-bottom:10px;height:50px}.anonim .toggle1{background:#ddd;width:60px;height:25px;border-radius:100px;display:block;appearance:none;-webkit-appearance:none;position:relative;cursor:pointer;float:right;margin-top:-5px}.anonim .toggle1:after{content:"";background:#999;display:block;height:30px;width:30px;border-radius:100%;position:absolute;left:0;transform:scale(.9);cursor:pointer;transition:all .4s ease;margin-top:-15px}.anonim .toggle1:checked{background:<?php echo $button_color;?>20;border:1px solid <?php echo $button_color;?>!important}.anonim .toggle1:checked:after{background:<?php echo $button_color;?>;left:28px}.comment textarea{margin-top:10px;line-height:1.2}.choose_payment.set_red,.form-control.set_red{border:1px solid #f15d5e!important;transition:all .1s ease}.card-group{margin-top:15px;min-height:175px}.donasiyuk-input .card-body{display:flow-root}.card-radio-btn input[type=radio]{display:none;opacity:0;width:0}.card-radio-btn .content_head{color:#333;font-size:16px;line-height:30px;font-weight:500}.card-radio-btn .content_sub{color:#9e9e9e;font-size:11px}.card-radio-btn .content_head.no_desc{padding-top:9px}.card-radio-btn .content_sub.no_desc{display:none}.card-radio-btn:nth-child(3n+1) .card-input-element+.card{clear:left}label.card-radio-btn:nth-child(3n) .card-input-element+.card{margin-right:0!important}.card-input-element+.card{width:28.7%;height:55px;margin:2%;justify-content:center;color:var(--primary);-webkit-box-shadow:none;box-shadow:none;border-radius:10px;text-align:center;border:1.5px solid #e2e8f0;box-shadow:none!important;-webkit-box-shadow:none!important;float:left;padding-top:5px;box-sizing:border-box!important;overflow:hidden!important;position:relative!important}.additional_nominal_value input, .other_nominal_value input, .pendapatan_emas input, .pendapatan_pertanian input, .pendapatan_perbulan input, .pendapatan_lainnya input, .pengeluaran input, .total_nisab_zakat input, .total_pendapatan input, .total_, .total_zakat input, .total_summary input{text-align:right;font-size:24px;font-weight:700;color:#23374d}.total_nisab_zakat input, .total_pendapatan input, .total_zakat input, .total_summary input{border:1px solid #edf7ff !important;background:#edf7ff;cursor:default;color:#4484c1;} 
        ...
        .total_zakat input:-moz-placeholder, .total_nisab_zakat input:-moz-placeholder, .total_pendapatan input:-moz-placeholder, .total_summary input:-moz-placeholder{font-size:16px;font-weight:400}.total_nisab_zakat input::placeholder, .total_pendapatan input::placeholder, .total_zakat input::placeholder, .total_summary input::placeholder{font-size:16px;font-weight:400;margin-top:-4px}.donasiyuk-input .filled{border:1px solid #c6d5e3!important}.card-input-element+.card:hover{cursor:pointer}.card-input-element:checked+.card{border:2px solid <?php echo $button_color;?>!important;-webkit-transition:border .3s;-o-transition:border .3s;transition:border .3s}.card-input-element:checked+.card .box-checklist,.card-input-element:checked+.card .box-checklist.no_desc{position:absolute!important;top:4px!important;right:4px!important;margin-top:0!important;padding-right:0!important;text-align:right}.card-input-element:checked+.card .box-checklist .checklist::after{content:"✓";color:#fff;font-style:normal;font-size:10px;font-weight:900;line-height:1;-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale;-webkit-animation-name:fadeInCheckbox;animation-name:fadeInCheckbox;-webkit-animation-duration:.3s;animation-duration:.3s;-webkit-animation-timing-function:cubic-bezier(.4,0,.2,1);animation-timing-function:cubic-bezier(.4,0,.2,1);background:<?php echo $button_color;?>;padding:2px 4px;border-radius:12px}@-webkit-keyframes fadeInCheckbox{from{opacity:0;-webkit-transform:rotateZ(-20deg)}to{opacity:1;-webkit-transform:rotateZ(0)}}@keyframes fadeInCheckbox{from{opacity:0;transform:rotateZ(-20deg)}to{opacity:1;transform:rotateZ(0)}}.box-char{text-align:right;font-size:11px}.donate_now{position:fixed;bottom:0;width:481px;margin-bottom:0}.donate_now .donation_button_now2{width:100%}.img-box{width:89%;padding:80px 20px 20px 25px;min-height:100px}.img-box img{width:160px;display:inline-block;position:absolute;border-radius:4px;box-shadow:0 8px 12px 0 rgba(0,0,0,.2)}.img-box div{font-size:12px;margin-left:180px;color:#aabdce}.img-box h1{font-size:16px;margin-left:180px;line-height:1.4}.donasi-loading{display:inline-block}.donasi-loading:after{content:" ";display:block;width:20px;height:20px;margin:0;border-radius:50%;border:4px solid #fff;border-color:#fff transparent #fff transparent;animation:donasi-loading 1.2s linear infinite;position:absolute;margin-top:-20px;margin-left:20px}@keyframes donasi-loading{0%{transform:rotate(0)}100%{transform:rotate(360deg)}}.loading-hide{display:none}.profile-picture{float:left;margin-bottom:30px}.profile-picture img{border-radius:120px;border:1px solid #dde4ec;width:70px;margin-left:10px}.profile-name{margin-left:110px;padding-top:18px;margin-bottom:50px}.profile-name .user-name{font-size:15px;font-weight:700}.profile-name .user-email,.profile-name .user-wa{font-style:italic;font-size:13px;padding-top:5px;color:#99a6bd}.charnum{margin-top: -20px;margin-bottom: 23px;margin-right: 10px;font-size: 10px;color: #acb2ca;}.tag-editor {list-style-type: none;padding: 10px 5px;margin: 0;overflow: hidden;border: 1px solid #eee;cursor: text;font: normal 14px sans-serif;color: #555;background: #fff;line-height: 20px;border-radius: 4px;}.tag-editor li {display: block;float: left;overflow: hidden;margin: 3px 0;}.tag-editor div {float: left;padding: 0 4px;}.tag-editor .placeholder {padding: 0 8px;color: #bbb;}.tag-editor .tag-editor-spacer {padding: 0;width: 8px;overflow: hidden;color: transparent;background: none;}.tag-editor input {vertical-align: inherit;border: 0;outline: none;padding: 0;margin: 0;cursor: text;font-family: inherit;font-weight: inherit;font-size: inherit;font-style: inherit;box-shadow: none;background: none;color: #444;}.tag-editor-hidden-src {position: absolute !important;left: -99999px;}.tag-editor ::-ms-clear {display: none;}.tag-editor .tag-editor-tag {padding-left: 5px;color: #46799b;background: #e0eaf1;white-space: nowrap;overflow: hidden;cursor: pointer;border-radius: 2px 0 0 2px;}.tag-editor .tag-editor-delete {background: #e0eaf1;cursor: pointer;border-radius: 0 2px 2px 0;padding-left: 3px;padding-right: 4px;}.tag-editor .tag-editor-delete i {line-height: 18px;display: inline-block;}.tag-editor .tag-editor-delete i:before {font-size: 16px;color: #8ba7ba;content: "×";font-style: normal;}.tag-editor .tag-editor-delete:hover i:before {color: #d65454;}.tag-editor .tag-editor-tag.active+.tag-editor-delete, .tag-editor .tag-editor-tag.active+.tag-editor-delete i {visibility: hidden;cursor: text;}.tag-editor .tag-editor-tag.active {background: none !important;padding: 0 !important;}.tag-editor .tag-editor-tag input {padding: 0px 10px !important;background: #F6FAFF;border-radius: 4px;height: 30px;margin-top:0px;}.tag-editor.set_red .placeholder div {color: #f15d5e !important;transition: all .1s ease;}.counter-number{display: flex;align-items: center;gap: 3px;}.counter-number .minus, .counter-number .plus, .counter-number .add{width: 17px;height: 17px;line-height: 15px;background: #f5f5f5;border-radius: 4px;padding: 8px 5px 8px 5px;border: 1px solid #eaeaea;vertical-align: middle;text-align: center;cursor:pointer;color:<?php echo $button_color;?>;}.counter-number .add {width: 80px;font-size: 12px;color: #fff;background: <?php echo $button_color;?>;border-color: <?php echo $button_color;?>;height: 16px;}.counter-number .add:hover {background: <?php echo $button_color;?>;}input.count {text-align: center;border:0px solid #ddd !important;border-radius:4px;display: inline-block;vertical-align: middle;height: 36px !important;width: 45px;font-size: 16px;font-weight: bold;padding:0;padding-top:3px;}input.count.filled {border:none !important;}.card-form .card {cursor: default !important;}.card-style .card-input-element:checked + .card{border: 2px solid <?php echo $button_color;?>;}.card-style .card-package .card-input-element:checked + .card{border: 2px solid transparent;}.card-style .card-input-element:checked + .card .box-checklist .checklist::after {background: <?php echo $button_color;?>;}.card-style .card-input-element:checked + .card.card-body {background: <?php echo $button_color;?>08;}.card-style .card-package .card-input-element:checked + .card.card-body {background: <?php echo $button_color;?>15;}.card-form.card-qurban .card-input-element:checked + .card, .card-form.card-package2 .card-input-element:checked + .card, .card-form.card-zfitrah .card-input-element:checked + .card {border: 2px solid transparent;}.card-form .card-input-element + .card {margin: 2% 1%;}.qurban_pricing span {padding-left:30px;}.package2_pricing span {padding-left:0px !important;}.card-form .card.card-body {height:auto;width:97%;text-align:left;padding-top: 10px; padding-bottom: 7px; padding-left:5px;}.card-form .content_head.no_desc {padding-top:5px;}.card-form .content_head.no_desc.qurban_pricing {font-size: 13px;padding-top:0 !important;}.card-form .content_head.no_desc.package2_pricing {font-size: 13px;padding-top:0 !important;}.card-form .card-img {width: 30%;float: left;}.card-form .card-img .img-qurban {height: 80px;width: 114px;border-radius: 6px;margin: 3px 5px 5px 8px;}.card-form .content_sub span {color: #818791;line-height: 1.5;}.counter-number {position: absolute;margin-left: -122px;margin-top: -39px;}.counter-number.btn_add{margin-left: -104px;margin-top: -40px;}.ripple {background-position: center;transition: background 0.2s;-webkit-user-select: none;-ms-user-select: none;user-select: none;}.ripple:hover {background: #F5F5F5 radial-gradient(circle, transparent 1%, #F5F5F5 0%) center/15000%;}.ripple:active {background-color: <?php echo $button_color;?>;background-size: 100%;transition: background 0s;scale: 0.85;}.scale_button:active {scale: 0.95;}.card-label:active {background: #0099ff1f;}.content_atasnama {padding:8px;margin-top:8px;}.qurban_pricing img {position: absolute;width: 20px;margin-top: 5px;}.qurban_pricing img.Kambing {position: absolute;width: 16px;margin-top: 6px;}.qurban_pricing img.Domba {position: absolute;width: 18px;margin-top: 6px;}.qurban_pricing img.Unta, .qurban_pricing img.Kerbau {position: absolute;width: 18px;margin-top: 6px;}.zfitrah_pricing {font-size: 14px !important;margin-top: -3px;}@media only screen and (max-width:480px){.card-form .card-img .img-qurban {height: 60px;width: 90px;margin-bottom: 0;}.card-form .content_head.no_desc {padding-top:0px;font-size: 15px;margin-bottom: -2px;}.card-form .content_head.no_desc.qurban_pricing {font-size: 12px;margin-top: -2px;}.card-form .card-img {width: 30%;}.card-form .card.card-body {padding-top:10px;padding-bottom:8px;padding-left:3px;}.counter-number {margin-left: -116px;gap: 1px;}#lala-alert-wrapper{margin-top:40px }.img-box img{width:130px }.img-box div{margin-left:140px }.img-box h1{margin-left:140px }.anonim label {font-size: 13px;}}@media only screen and (max-width:380px){.card-form .card-img .img-qurban {height: 55px;width:78px;margin-bottom: 0;}input.count {width:40px;}.counter-number {gap:0px;margin-left: -105px;}}
            ...
            .donasiyuk-input p img { width:100%;}
            .donasiyuk-input.sapaan {display:flex!important;align-items:center!important;gap:14px!important;width:100%!important;margin:16px 0!important;clear:both!important}
            group.section-sapaan-label {width:auto!important;min-width:68px!important;flex-shrink:0!important;float:none!important;margin:0!important;display:flex!important;align-items:center!important}
            group.section-sapaan-label p {margin:0!important;padding:0!important;font-size:14px!important;font-weight:500!important;color:#333!important;white-space:nowrap!important;line-height:1!important}
            group.section-sapaan-text {flex:1!important;width:auto!important;display:flex!important;align-items:center!important;margin:0!important}
            .sapaan .input-container {height:38px!important;line-height:38px!important;width:100%!important;text-align:center!important;position:relative!important;margin:0!important;flex:1!important}
            .sapaan .input-container:first-child label {border-radius:6px 0 0 6px!important}
            .sapaan .input-container:last-child label {border-radius:0 6px 6px 0!important;border-right:1px solid #e5e8ec!important}
            .sapaan input[type=radio] {cursor:pointer!important;opacity:0!important;width:100%!important;height:38px!important;position:relative!important;z-index:1!important;margin:0!important}
            .sapaan label {width:100%!important;height:100%!important;position:absolute!important;border:1px solid #e5e8ec!important;border-right:inherit!important;top:0!important;left:0!important;font-size:14px!important;font-weight:500!important;color:#64748b!important;display:flex!important;align-items:center!important;justify-content:center!important;transition:all .2s ease!important}
            .sapaan input:checked + label {background-color:<?php echo $button_color;?>!important;border-color:<?php echo $button_color;?>!important;color:#fff!important;font-weight:600!important;z-index:2!important}
            .option_zakat input[type=radio] {cursor:pointer;opacity:0;width:100%;height:42px;background-color:blue;position:relative;z-index:1}
            .option_zakat group {width:100%;display:flex}
            .option_zakat .input-container {height:36px;line-height:36px;width:100%;text-align:center;position:relative;margin-bottom:10px;margin-top:15px}
            .option_zakat .input-container:first-child label {border-radius:5px 0 0 5px}
            .option_zakat .input-container:last-child label {border-radius:0 5px 5px 0;border-right:1px solid #e5e8ec}
            .option_zakat label {width:100%;height:100%;position:absolute;border:1px solid #e5e8ec;border-right:inherit;top:0;left:0;font-family:arial;color:#737373}
            .option_zakat input:checked + label {background-color:<?php echo $button_color;?>;top:0;left:0;border:1px solid <?php echo darken_color($button_color, $darker=1.25);?>!important;z-index:2;color:#fff}
		...
		/* --- Modern Sticky Bottom Donate Button (#fixed-button / .donate_now) --- */
		#fixed-button,
		#fixed-button.donate_now,
		.donate_now {
			position: fixed !important;
			bottom: 0 !important;
			left: 50% !important;
		...
		#fixed-button span,
		.donate_now span,
		#fixed-button span.button-disabled,
		.donate_now span.button-disabled {
			width: 100% !important;
			display: block !important;
			flex: 1 !important;
			margin: 0 !important;
		...
		.donate_now .donation_button_now2,
		#fixed-button button.donation_button_now2,
		.donate_now button.donation_button_now2 {
			width: 100% !important;
...
<body>
        ...
        <div class="popup-captcha-overlay"></div>
            <div class="popup-content" id="popupCaptcha">
                ...
                <button class="donation_button_now3 scale_button" data-popup="false">
                  <span style="transition:all .5s ease"><?php echo esc_html(isset($popup_info_button) ? $popup_info_button : 'Next'); ?></span>
	...
	<div id="header-title" class="section-box">
		<button type="button" class="dyk-sticky-back-btn" onclick="if(history.length > 1){history.back();}else{window.location.href='<?php echo $back_urlnya; ?>';}" title="Kembali">
	...
	<!-- Unified Campaign Header Card (Gambar & Nama Donasi Menyatu) -->
	<div class="section-box dyk-form-header-card">
		<?php if($row->image_url!=null){?>
		<div class="section-image">
			<button type="button" class="dyk-hero-back-btn" onclick="if(history.length > 1){history.back();}else{window.location.href='<?php echo $back_urlnya; ?>';}" title="Kembali">
			...
			<img alt="Image" class="parallax" src="<?php echo $row->image_url; ?>">
		</div>
		<?php }else{?>
		<div class="section-image">
			<button type="button" class="dyk-hero-back-btn" onclick="if(history.length > 1){history.back();}else{window.location.href='<?php echo $back_urlnya; ?>';}" title="Kembali">
			...
			<img alt="Image" class="parallax" src="<?php echo plugin_dir_url( __FILE__ ).'admin/images/cover_donasiyuk.jpg'; ?>">
		</div>
		<?php } ?>

		<div class="dyk-program-info-box">
			<div class="dyk-program-info-label"><?php echo $text3; ?></div>
			<h1 class="dyk-program-info-title"><?php echo $campaign_title; ?></h1>
		</div>
	</div>
	...
	<div class="section-box main-box">
			<div class="form-group" id="form-group">
				...
				<div class="donasiyuk-input" style="margin-top: 10px;">
				...
				<div class="donasiyuk-input other_nominal_value hide_input">
                    ...
                    foreach ($additional_formula['data'] as $key => $value) { ?>
                        ...
                        <div class="container_additional_value">
                            <div class="donasiyuk-input">
                                <label><?php echo $value['label']; ?></label><br>
                            </div>
                            <div class="donasiyuk-input additional_nominal_value add_donate<?php echo $no_field; ?>">
                    ...
                    <div>
                        ...
                        <div class="donasiyuk-input total_summary" style="margin-bottom: 40px;">
				...
				<div class="donasiyuk-input other_nominal_value">
                ...
                if($row->zakat_penghasilan_custom_title=='1') {
                    if(!empty($row->pendapatan1_title)){
                        $pendapatan1_title = $row->pendapatan1_title;
                    }
                    if(!empty($row->pendapatan2_title)){
                        $pendapatan2_title = $row->pendapatan2_title;
                    }
                    if(!empty($row->pengeluaran_title)){
                ...
                <?php if($row->zakat_penghasilan_type=='pertanian'){ ?>
                    ...
                    <div class="donasiyuk-input option_zakat" style="width:100% !important;margin-bottom:20px;">
                    ...
                    <div class="donasiyuk-input pendapatan_pertanian">
                ...
                <?php }elseif($row->zakat_penghasilan_type=='emas'){ ?>
                    ...
                    <div class="donasiyuk-input pendapatan_emas">
                ...
                <?php }else{ ?>
                    ...
                    <div class="donasiyuk-input pendapatan_perbulan">
                    ...
                    <div class="donasiyuk-input pendapatan_lainnya">
                        <input placeholder="Opsional, jika ada" type="tel" class="form-control" name="pendapatan_lainnya" value="">
                        <div class="currency"><?php echo $show_currency; ?></div>
                    </div>

                    <div class="donasiyuk-input" <?php echo $show_pengeluaran; ?>>
                        <label><?php echo $pengeluaran_title; ?></label><br>
                    </div>

                    <div class="donasiyuk-input pengeluaran" <?php echo $show_pengeluaran; ?>>
                ...
                <div style="background: #edf7ff99;padding: 15px 15px;border-radius:4px;margin-bottom: 20px;border: 1px solid #ddeaf6;margin-top: 30px;">
                    ...
                    <div class="donasiyuk-input total_pendapatan" style="margin-bottom: 20px;">
                    ...
                    <div class="donasiyuk-input total_nisab_zakat" style="margin-bottom: 20px;">
                    ...
                    <div class="donasiyuk-input total_zakat" style="margin-bottom: 20px;">
				...
				<div class="donasiyuk-input dyk-payment-field">
					<label class="dyk-field-label">Metode Pembayaran</label>
					<div class="dyk-select-container" id="dykSelectContainer">
						<div id="choose_payment" class="dyk-select-trigger">
							<div class="dyk-trigger-left">
								<div class="box_img_payment">
									<img class="img_payment_selected" src="<?php echo plugin_dir_url( __FILE__ ) . 'assets/images/bank/bank.png';?>" alt="Bank" data-paymentmethod="" data-paymentcode="" data-paymentnumber="" data-paymentaccount="">
								</div>
								<span class="title_payment selected">Pilih Metode Pembayaran</span>
							</div>
							<div class="dyk-trigger-arrow">
								<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg>
							</div>
						</div>

						<div class="dyk-dropdown-menu" id="dykDropdownMenu">
							<div class="card_payment">
								...
								if($row->payment_status=='1'){ 
									...
									$urutkan = explode(',', $payment_mapping);
									$mapUrutan = [];
									...
									foreach ($urutkan as $nomor) {
										...
										if (isset($data_bank[$key])) {
											$payment_setting[$key] = $data_bank[$key];
										}
									}
									...
									foreach($payment_setting as $key => $value){
										$code_method = $value[0];
										$status_active = '0';
										if($code_method=='instant'){ $status_active = $instant_setting; }
										if($code_method=='va'){ $status_active = $va_setting; }
										if($code_method=='transfer'){ $status_active = $transfer_setting; }
								...
								?>
											<label class="card-label title-list <?php echo 'payment_'.$code_method; ?>" style="<?php echo $margin_top.$hide_va_account; ?>">
												<span class="card-title2"><?php echo $value[1]?></span>
											</label>
												...
												if($urutan_code_bank==$payment_urutan){
													foreach ($payment_list as $val2) {
														if($val2->code==$payment_code){
															...
															if($code_method=='va'){ $payment_name = 'VA '.$val2->name; }
															if($code_method=='transfer'){
																if($payment_number=='flip'){
																	$payment_name =  $val2->name;
																}else{
																	if($currency=='MYR'){ $payment_name = $val2->name; }
																	else{ $payment_name = 'Transfer '.$val2->name; }
																}
																if($val2->name == 'Credit Card'){ $payment_name = 'Credit Card'; }
																if($val2->name == 'Paypal'){ $payment_name = 'Paypal'; }
																if($val2->name == 'Stripe'){ $payment_name = 'Stripe'; }
															}
														}
														if($payment_account=='midtrans' || $payment_account=='tripay' || $payment_account=='ipaymu' || $payment_account=='flip' || $payment_account=='remitcepat' || $payment_account=='duitku' || $payment_account=='xendit'){
															$flag_pg = 'set_pg';
														}else{
															$flag_pg = 'set_manual';
														}
                ...
                <div class="donasiyuk-input sapaan" style="<?php if($form_sapaan_setting!='1'){echo'display:none;';} if($set_user==true && $sapaan!=''){echo'display:none;';}?>">
                ...
                <?php if($set_user==true) { ?>
                <div id="box_update_data" class="donasiyuk-input" style=" background: #fff;border: 1px solid #e5e8ec; padding: 20px 20px 25px 20px;   border-radius: 4px;   margin-bottom: 25px;<?php if($fullname=='' || $user_wa==''){}else{echo'display:none;';} ?>">

                    <div class="donasiyuk-input fullname_update">
                    ...
                    <div class="donasiyuk-input whatsapp_update">
                    ...
                    <button class="donation_button_now4" id="update_data" style="background:<?php echo $button_color; ?>;border-color:<?php echo $button_color; ?>;width: 100%;">Update<div class="donasi-loading loading-hide"></div></button>

                </div>
                <?php } ?>

                <div class="donasiyuk-input fullname">
				...
				<div class="donasiyuk-input anonim" <?php if($form_anonim_setting=='0'){echo'style="display:none;"';}?>>
					<label>Sembunyikan nama saya (<?php echo $anonim_text; ?>)</label>
					<input id="anonim" type="checkbox" class="toggle1" name="anonim" /></span>
				</div>
				
				<div class="donasiyuk-input whatsapp">
				...
				<div class="donasiyuk-input email">
                ...
                foreach ($additional_field['data'] as $key => $value) { ?>
                    ...
                    <div class="donasiyuk-input input-text">
                    ...
                    <div class="donasiyuk-input input-number">
                    ...
                    <div class="donasiyuk-input textarea_<?php echo $no_field; ?>">
                ...
                <div class="donasiyuk-input comment">
				...
				<div class="powered-donasiyuk-box" style="margin-top: -10px; margin-bottom: 75px; text-align: center; font-size: 11px; color: #94a3b8;">Powered by DonasiYuk</div>
				<?php } ?>
			</div>
...
<script>
		...
		$(document).ready(function() {
            ...
            */ ?>
            ...
            $(".donation_button_now2, .donation_button_now3").on("click", function(e) {
                ...
                if(name==''){
                    $('.donasiyuk-input.fullname input').addClass('set_red');
                }
                if(whatsapp==''){
                    $('.donasiyuk-input.whatsapp input').addClass('set_red');
                }
            ...
            function submit_donation(el){
                ...
                if(main_donate>=min_donasi){
                    ...
                    var ip = "<?php echo donasiyuk_getIP();?>";
                    var os = "<?php echo donasiyuk_getOS();?>";
                    var browser = "<?php echo donasiyuk_getBrowser();?>";
                    var mobdesktop = "<?php echo donasiyuk_getMobDesktop();?>";
                    ...
                    var data = {
                        "action": "dykfunction_submit_donasi",
                        "datanya": data_nya
                    };
            ...
            $('#whatsapp').on('blur', function () {
                ...
                $('.donasiyuk-input.whatsapp .checking-status').html('<div class="checking">Checking whatsapp number...</div>');
                ...
                var data = {
                    "action": "dykfunction_check_wa_exist",
                    "datanya": data_nya
                };
                ...
                jQuery.post("<?php echo $home_url; ?>/wp-admin/admin-ajax.php", data, function(response) {
                    ...
                    if(response=='exist'){
                        whatsapp_valid = true;
                        $('.donasiyuk-input.whatsapp .checking-status').html('<div class="checking valid">Whatsapp number valid.</div>');
                    }else if(response=='error'){
                        $('.donasiyuk-input.whatsapp .checking-status').html('<div class="checking not-valid-error">Error, coba ulangi kembali ketik nomor whatsapp anda.</div>');
                    }else if(response=='wanotif_off'){
                        $('.donasiyuk-input.whatsapp .checking-status').html('<div class="checking not-valid-error">Wanotif API tidak aktif. Silakan periksa lalu coba lagi.</div>');
                    }else{
                        $('.donasiyuk-input.whatsapp .checking-status').html('<div class="checking not-valid">Whatsapp number not valid.</div>');
                    }
            ...
            // Toggle Hybrid Payment Dropdown
            $(document).on("click", "#dykSelectTrigger, #choose_payment", function(e) {
                e.preventDefault();
                e.stopPropagation();
                $("#dykSelectContainer").toggleClass("open");
            });

            // Select Payment Channel from Dropdown
            $(document).on("click", ".dyk-dropdown-menu .card-label:not(.title-list)", function(e) {
                var $label = $(this);
                var payment_method = $label.attr("data-method");
                var payment_code = $label.attr("data-code");
                var payment_name = $label.attr("data-paymentname");
                var payment_number = $label.attr("data-number");
                var payment_account = $label.attr("data-account");

                $label.find("input.card-radio").prop("checked", true);
                $(".dyk-dropdown-menu .card-label").removeClass("active");
                $label.addClass("active");

                $(".title_payment").text(payment_name).css({"font-weight":"600"});
                $(".box_img_payment img").attr("src", "<?php echo plugin_dir_url( __FILE__ ) . 'assets/images/bank/';?>" + payment_code + ".png");
                $(".box_img_payment img").attr("data-paymentmethod", payment_method)
                                        .attr("data-paymentcode", payment_code)
                                        .attr("data-paymentnumber", payment_number)
                                        .attr("data-paymentaccount", payment_account);
                $("#choose_payment").removeClass("set_red");
                $("#dykSelectContainer").removeClass("open");
            });

            // Click outside to close Dropdown
            $(document).on("click", function(e) {
                if (!$(e.target).closest("#dykSelectContainer").length) {
                    $("#dykSelectContainer").removeClass("open");
                }
            });
            ...
            <?php if($set_user==true) { ?>
                ...
                $('#update_data').bind('click', function() {
                    ...
                    var data = {
                        "action": "dykfunction_update_name_whatsapp",
                        "datanya": data_nya
                    };
    ...
    $(function() {
        ...
        var headerTitle = $('.campaign-header-title');
        $(window).scroll(function() {
            var scroll = $(window).scrollTop();
            if (scroll >= 150) {
                header.addClass("flying-header");
                headerTitle.addClass("show-title");
            } else {
                header.removeClass("flying-header");
                headerTitle.removeClass("show-title");
            }
        });
    });
	</script>
