/**
 * DonasiYuk Core Scripts v2.2.5
 */
(function($) {
    "use strict";

    console.log("DonasiYuk 2.2.5");

    var delimiter_on_donasiaja = typeof donasiyukObjName !== 'undefined' && donasiyukObjName.delimiter ? donasiyukObjName.delimiter : '.';
    var currency = typeof donasiyukObjName !== 'undefined' && donasiyukObjName.show_currency ? donasiyukObjName.show_currency : 'Rp';

    $.fn.onEnter = function(a) {
        return this.bind("keypress", function(t) {
            if (13 == t.keyCode) a.apply(this, [t]);
        }), this;
    };
    ...
    window.run_other_nominal = run_other_nominal;

    function run_zakat(el) {
        var show_currency = currency;
        var zakat_pengairan = 'mandiri';
        var zakat_nisab_kg = 520;
        var zakat_percent = 2.5;
        var total_pendapatan = 0;
        var nisabnya = 0;
        var randchar = '';
        var zakattype = '';
        var data_id_form = $(el).attr('data-id');
        var zakat_penghasilan_type = $(el).attr('data-zakat_penghasilan_type');

        if (zakat_penghasilan_type == 'pertanian') {
            var zakat_harga_per_kg = $(el).attr('data-zakat_harga_per_kg') || 0;
            zakat_pengairan = $(el).attr('data-zakat_pengairan');
            zakat_nisab_kg = $(el).attr('data-zakat_nisab_kg') || 520;
            ...
            var pendapatan_pertanian_int = (pendapatan_pertanian && pendapatan_pertanian != '') ? pendapatan_pertanian.replace(/\./g, '') : 0;

            total_pendapatan = parseInt(pendapatan_pertanian_int || 0) * parseInt(zakat_harga_per_kg || 0);
            $('#shortform_' + data_id_form + ' .total_pendapatan input').val(numberWithDot(total_pendapatan));

            nisabnya = zakat_nisab_kg * zakat_harga_per_kg;

            var option_zakat = $('#shortform_' + data_id_form + ' input[type="radio"][name="option_zakat_' + randchar + '"]:checked').val();
            ...
            var pendapatan_emas_int = (pendapatan_emas && pendapatan_emas != '') ? pendapatan_emas.replace(/\./g, '') : 0;

            total_pendapatan = parseInt(pendapatan_emas_int || 0) * parseInt(harga_emas || 0);
            $('#shortform_' + data_id_form + ' .total_pendapatan input').val(numberWithDot(total_pendapatan));

            nisabnya = 85 * harga_emas;

            var option_zakat = $('input[type="radio"][name="option_zakat_' + randchar + '"]:checked').val();
            $('#shortform_' + data_id_form + ' .total_nisab_zakat input').val(numberWithDot(nisabnya));
            ...
            var harga_emas = (emas_per_gram && emas_per_gram != 0) ? emas_per_gram : 2450000;

            var pendapatan_perbulan = $('#shortform_' + data_id_form + ' .pendapatan_perbulan input').val();
            var pendapatan_perbulan_int = (pendapatan_perbulan && pendapatan_perbulan != '') ? pendapatan_perbulan.replace(/\./g, '') : 0;

            var pendapatan_lainnya = $('#shortform_' + data_id_form + ' .pendapatan_lainnya input').val();
            var pendapatan_lainnya_int = (pendapatan_lainnya && pendapatan_lainnya != '') ? pendapatan_lainnya.replace(/\./g, '') : 0;

            var pengeluaran = $('#shortform_' + data_id_form + ' .pengeluaran input').val();
            var pengeluaran_int = (pengeluaran && pengeluaran != '') ? pengeluaran.replace(/\./g, '') : 0;

            total_pendapatan = parseInt(pendapatan_perbulan_int || 0) + parseInt(pendapatan_lainnya_int || 0) - parseInt(pengeluaran_int || 0);
            $('#shortform_' + data_id_form + ' .total_pendapatan input').val(numberWithDot(total_pendapatan));
        ...
        var total_zakat = Math.round((zakat_percent * total_pendapatan) / 100);
    ...
    window.run_zakat = run_zakat;

    $(document).ready(function() {
        $(".load_campaign").bind("click", function() {
            var t = $(this).attr("data-id"),
                e = $(this).attr("data-count"),
                d = $(this).attr("data-date");
            $(this).text("Load more...");
            var i = {
                action: "donasiyuk_load_campaign",
                datanya: [t, e, "", d]
            };
            var ajaxUrl = typeof donasiyukObjName !== 'undefined' ? donasiyukObjName.varSiteUrl + "/wp-admin/admin-ajax.php" : "/wp-admin/admin-ajax.php";
            $.post(ajaxUrl, i, function(i) {
                if ("" == i) {
                    $("#box_button_" + t + " .donasiyuk_loadmore_info").text("No more data").slideDown();
                    setTimeout(function() {
                        $("#box_button_" + t + " .donasiyuk_loadmore_info").hide();
                    }, 5000);
                    $("#box_button_" + t + " button").text("Load more");
                } else {
                    e = parseFloat(e) + 1;
                    $("#box_button_" + t + " button").attr("data-count", e);
                    $("#box_button_" + t + " button").text("Load more");
                    $("#section_" + t).append(i);
                }
            });
        ...
        $(".load_list_donatur").bind("click", function(t) {
            var e = $(this).attr("id"),
                i = $(this).attr("data-id"),
                n = $(this).attr("data-count"),
                o = $(this).attr("data-anonim"),
                s = $(this).attr("data-fullanonim");
            $("#" + e).text("Load more...");
            var d = {
                action: "dykfunction_load_list_donatur",
                datanya: [e, i, n, o, s]
            };
            var ajaxUrl = typeof donasiyukObjName !== 'undefined' ? donasiyukObjName.varSiteUrl + "/wp-admin/admin-ajax.php" : "/wp-admin/admin-ajax.php";
            $.post(ajaxUrl, d, function(t) {
                if ("" == t) {
                    $("#box_btn_" + e + " .loadmore_info").html("No more data").slideDown();
                    setTimeout(function() {
                        $("#box_btn_" + e + " .loadmore_info").hide();
                    }, 5000);
                }
                n = parseFloat(n) + 1;
                $("#" + e).attr("data-count", n).text("Load more");
                $("#box_" + e).append(t);
            });
        });

        var $timelineExpandableTitle = $(".timeline-action.is-expandable .title");
        $($timelineExpandableTitle).attr("tabindex", "0");
        $(".timeline").each(function(t, e) {
            var i = $(e).find(".timeline-action.is-expandable");
            $(i).each(function(e, i) {
                var n = $(i).find(".content");
                $(n).attr("id", "timeline-" + t + "-milestone-content-" + e).attr("role", "region");
                $(n).attr("aria-expanded", $(i).hasClass("expanded"));
                $(i).find(".title").attr("aria-controls", "timeline-" + t + "-milestone-content-" + e);
            });
        });
        ...
        $($timelineExpandableTitle).keyup(function(t) {
            if (13 == t.which) {
                $(this).click();
            } else if (37 == t.which || 38 == t.which) {
        ...
        $(".donasiyuk_search input").click(function(t) {
        ...
        $(document).on("keypress", ".input-search", function(t) {
            if (13 == t.which) {
                var e = $(this).val();
                var siteUrl = typeof donasiyukObjName !== 'undefined' ? donasiyukObjName.varSiteUrl : '';
                var i = siteUrl + "/search_campaign/s?s=" + e;
                window.open(i, "_self").location;
            }
        });

        $(".donasiyuk_search").attr("data-action", "run_search_donasiyuk");
        var t = $(".donasiyuk_search").attr("data-action");
        if ("run_search_donasiyuk" == t) {
            var e = $("head"),
                i = e.find("link[rel='stylesheet']:last"),
                n = "<link rel='stylesheet' type='text/css' href='https://fonts.googleapis.com/icon?family=Material+Icons'>";
            if (i.length) {
                i.after(n);
            } else {
                e.append(n);
            }
            $("body").append('<div class="donasiyuk_search_box"><div class="control"><div class="btn-material"></div></div><i class="icon-close material-icons">close</i><div class="search-input"><input class="input-search" placeholder="Start Typing" type="text" value=""></div></div>');
        }

        $(".cards__campaign").each(function() {
            var idcard = this.id;
            if ($(".cards__campaign").hasClass("campaign_grid")) {
                var width_box_campaign = $('.cards__campaign').width();
                if (width_box_campaign < 540) {
                    $('#' + idcard).addClass('set50');
                }
            }
        });
        ...
        $('select#jumlah_paket').on('change', function(e) {
            ...
            if (jumlah != '0') {
                ...
                var content = nominal.toString().replace(/\B(?=(\d{3})+(?!\d))/g, delimiter_on_donasiaja);
        ...
        $('.other_nominal_value input').on('keyup', function(e) {
            ...
            $(this).val(function(index, value) {
                return value.replace(/\D/g, "").replace(/\B(?=(\d{3})+(?!\d))/g, delimiter_on_donasiaja);
            });
        ...
        $(document).on("keyup", ".pendapatan_emas input, .pendapatan_pertanian input, .pendapatan_perbulan input, .pendapatan_lainnya input, .pengeluaran input", function(e) {
            if (e.which >= 37 && e.which <= 40) return;
            $(this).val(function(index, value) {
                return value.replace(/\D/g, "").replace(/\B(?=(\d{3})+(?!\d))/g, delimiter_on_donasiaja);
            });
